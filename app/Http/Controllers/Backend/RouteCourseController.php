...
class RouteCourseController extends Controller {
    ...
    public function index() {
        $records = Course::orderBy('ordering','desc')->where('is_pro',1)->get();
        return view('backend/route_course/index', compact('records'));
    }

    public function create() {
          $teachers = DB::table('teacher')->get();
        $studies = DB::table('study')->get();
        $count_ordering = Course::where('is_pro',1)->count();
        return view('backend/route_course/create',compact('count_ordering','teachers','studies'));
    }
    ...
    public function store(Request $request) {
        ...
        if ($res) {
            return redirect()->route('admin.route_course.index')->with('success', 'Tạo mới thành công');
        } else {
            return redirect()->route('admin.route_course.index')->with('error', 'Tạo mới thất bại');
        }
    }
    ...
    public function edit($id) {
       ...
       if($record){
        ...
        return view('backend/route_course/edit', compact('record','teachers','studies'));
        }else{
            abort(404);
        }
    }
    ...
    public function update_multiple(Request $request) {
        ...
        if($request->action == "save"){      
          $records =Course::where('is_pro',1)->orderBy('ordering','desc')->get();
