...
if (preg_match('/(tablet|ipad|playbook)|(android(?!.*(mobi|opera mini)))/i', strtolower(filter_input(INPUT_SERVER, 'HTTP_USER_AGENT')))) {
    $tablet_browser++;
}

if (preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|phone|android|iemobile)/i', strtolower(filter_input(INPUT_SERVER, 'HTTP_USER_AGENT')))) {
    $mobile_browser++;
}
...
$mobile_ua = strtolower(substr(filter_input(INPUT_SERVER, 'HTTP_USER_AGENT'), 0, 4));
$mobile_agents = array(
...
if (strpos(strtolower(filter_input(INPUT_SERVER, 'HTTP_USER_AGENT')), 'opera mini') > 0) {
    $mobile_browser++;
