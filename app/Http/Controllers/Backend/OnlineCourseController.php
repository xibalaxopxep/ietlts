...
class OnlineCourseController extends Controller {
    ...
    public function index() {
        $records = Course::orderBy('ordering','desc')->where('is_online',1)->get();
        return view('backend/online_course/index', compact('records'));
    }

    public function create() {
        $teachers = DB::table('teacher')->get();
        $studies = DB::table('study')->get();
        $count_ordering = Course::where('is_online',1)->count();
        return view('backend/online_course/create',compact('count_ordering','teachers','studies'));
    }
    ...
    public function store(Request $request) {
        ...
        if ($res) {
            return redirect()->route('admin.online_course.index')->with('success', 'Tạo mới thành công');
        } else {
            return redirect()->route('admin.online_course.index')->with('error', 'Tạo mới thất bại');
        }
    }
    ...
    public function edit($id) {
       ...
       if($record){
        ...
        return view('backend/online_course/edit', compact('record','teachers','studies'));
        }else{
            abort(404);
        }
    }
    ...
    public function update_multiple(Request $request) {
        ...
        if($request->action == "save"){      
          $records =Course::where('is_online',1)->orderBy('ordering','desc')->get();
